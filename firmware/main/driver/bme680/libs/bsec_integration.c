 ...
 *               Please call this function before processing of data using
 * bsec_do_steps function
 *
 * @param[in]    sample_rate         mode to be used (either
 * BSEC_SAMPLE_RATE_ULP or BSEC_SAMPLE_RATE_LP)
 *
 * @return       subscription result, zero when successful
 */
static bsec_library_return_t bme68x_bsec_update_subscription(
    float sample_rate) {
 ...
 *
 * @param[in]   sample_rate         mode to be used (either BSEC_SAMPLE_RATE_ULP
 * or BSEC_SAMPLE_RATE_LP)
 * @param[in]   temperature_offset  device-specific temperature offset (due to
 * self-heating)
 * @param[in]   bus_write           pointer to the bus writing function
 * @param[in]   bus_read            pointer to the bus reading function
 * @param[in]   sleep               pointer to the system specific sleep
 * function
 * @param[in]   state_load          pointer to the system-specific state load
 * function
 * @param[in]   config_load         pointer to the system-specific config load
 * function
 * @param[in]   dev         		pointer to the sensor communication and
 * inventory details strucuture
 *
 * @return      zero if successful, negative otherwise
 */
return_values_init bsec_iot_init(float sample_rate, float temperature_offset,
                                 bme68x_write_fptr_t bus_write,
                                 bme68x_read_fptr_t bus_read, sleep_fct sleep,
                                 state_load_fct state_load,
  ...
  if (bsec_config_len != 0) {
    ret.bsec_status = bsec_set_configuration(bsec_config, bsec_config_len,
  ...
  if (bsec_state_len != 0) {
    ret.bsec_status = bsec_set_state(bsec_state, bsec_state_len, work_buffer,
                                     sizeof(work_buffer));
    if (ret.bsec_status != BSEC_OK) {
      return ret;
    }
...
/*!
 * @brief       This function is written to process the sensor data for the
 * requested virtual sensors
 *
 * @param[in]   bsec_inputs         input structure containing the information
 * on sensors to be passed to do_steps
 * @param[in]   num_bsec_inputs     number of inputs to be passed to do_steps
 * @param[in]   output_ready        pointer to the function processing obtained
 * BSEC outputs
 *
 * @return      library function return codes, zero when successful
 */
static bsec_library_return_t bme68x_bsec_process_data(
    bsec_input_t *bsec_inputs, uint8_t num_bsec_inputs,
  ...
  if (num_bsec_inputs > 0) {
    ...
    /* Perform processing of the data by BSEC
       Note:
       * The number of outputs you get depends on what you asked for during
       bsec_update_subscription(). This is handled under
       bme68x_bsec_update_subscription() function in this example file.
       * The number of actual outputs that are returned is written to
       num_bsec_outputs. */
     ...
     */
    output_ready(timestamp, gas_estimate_1, gas_estimate_2, gas_estimate_3,
...
/*!
 * @brief       Read the data from registers and populate the inputs structure
 * to be passed to do_steps function
 *
 * @param[in]   currTimeNs      		system timestamp value passed
 * for processing data
 * @param[in]   data                  	input structure that contains the gas
 * sensor data to be passed to process data
 * @param[in]   bsec_process_data       process data variable returned from
 * sensor_control
 * @param[in]   output_ready        	pointer to the function processing
 * obtained BSEC outputs
 *
 * @return      function result, one when successful & zero when unsuccessful
 */
uint8_t processData(int64_t currTimeNs, struct bme68x_data data,
                    int32_t bsec_process_data, output_ready_fct output_ready) {
  ...
  if (nInputs > 0) {
    ...
    if (bsec_status != BSEC_OK) return 0;
  }
  return 1;
}
 ...
 *
 * @param[in]   mode      			sensor operation mode to
 * calculate the shared heater duration
 *
 * @return      calculated duration
 */
uint32_t getMeasDur(uint8_t mode) {
  if (mode == BME68X_SLEEP_MODE) mode = lastOpMode;
 ...
 *
 * @param[in]   sensor_settings     settings of the bme68x sensor adopted by
...
void setBme68xConfigParallel(bsec_bme_settings_t *sensor_settings) {
  ...
  sharedHeaterDur =
      BSEC_TOTAL_HEAT_DUR - (getMeasDur(BME68X_PARALLEL_MODE) / INT64_C(1000));
...
/*!
 * @brief       Runs the main (endless) loop that queries sensor settings,
 * applies them, and processes the measured data
 *
 * @param[in]   sleep               pointer to the system specific sleep
 * function
 * @param[in]   get_timestamp_us    pointer to the system specific timestamp
 * derivation function
 * @param[in]   output_ready        pointer to the function processing obtained
 * BSEC outputs
 * @param[in]   state_save          pointer to the system-specific state save
 * function
 * @param[in]   save_intvl          interval at which BSEC state should be saved
 * (in samples)
 *
 * @return      none
 */
void bsec_iot_loop(sleep_fct sleep, get_timestamp_us_fct get_timestamp_us,
                   output_ready_fct output_ready, state_save_fct state_save,
                   uint32_t save_intvl) {
  /* Timestamp variables */
  ...
  while (1) {
    ...
    if (time_stamp >= sensor_settings.next_call) {
      /* Retrieve sensor settings to be used in this time instant by calling
       * bsec_sensor_control */
      ...
      if (n_samples >= save_intvl) {
        bsec_status =
            bsec_get_state(0, bsec_state, sizeof(bsec_state), work_buffer,
                           sizeof(work_buffer), &bsec_state_len);
        if (bsec_status == BSEC_OK) {
